...
@pytest.mark.skipif(
    get_video_backend() != "pyav" and not io._HAS_VIDEO_OPT,
...
class TestVideo:
    ...
    @pytest.mark.skipif(
        not io._HAS_VIDEO_OPT, reason="video_reader backend is not chosen"
    )
    ...
    def test_read_timestamps(self):
        with temp_video(10, 300, 300, 5) as (f_name, data):
            ...
            with av.open(f_name) as container:
                ...
                num_frames = int(
                    round(
                        float(stream.average_rate * stream.time_base * stream.duration)
                    )
                )
    ...
    def test_read_timestamps_pts_unit_sec(self):
        with temp_video(10, 300, 300, 5) as (f_name, data):
            ...
            with av.open(f_name) as container:
                ...
                expected_pts = [
                    i * pts_step * stream.time_base for i in range(num_frames)
                ]
    ...
    def test_read_partial_video_pts_unit_sec(self, start, offset):
        with temp_video(10, 300, 300, 5, lossless=True) as (f_name, data):
            ...
            lv, _, _ = io.read_video(
                f_name, pts[start], pts[start + offset - 1], pts_unit="sec"
            )
            s_data = data[start : (start + offset)]
            assert len(lv) == offset
            assert_equal(s_data, lv)
    ...
    @pytest.mark.skipif(
        sys.platform == "win32", reason="temporarily disabled on Windows"
    )
    def test_write_video_with_audio(self):
        ...
        with get_tmp_dir() as tmpdir:
            ...
            assert (
                pytest.approx(out_audio_stream.frames, rel=0.0, abs=1)
                == audio_stream.frames
            )
