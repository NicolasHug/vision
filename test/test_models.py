...
def get_available_quantizable_models():
    ...
    return [
        k
        for k, v in models.quantization.__dict__.items()
        if callable(v) and k[0].lower() == k[0] and k[0] != "_"
    ]
...
def _assert_expected(output, name, prec):
    ...
    if ACCEPT:
        ...
        if binary_size > MAX_PICKLE_SIZE:
            raise RuntimeError(
                "The output for {}, is larger than 50kb".format(filename)
            )
    else:
        expected = torch.load(expected_file)
        rtol = atol = prec
        torch.testing.assert_close(
            output, expected, rtol=rtol, atol=atol, check_dtype=False
        )
...
_model_params = {
    ...
    "fasterrcnn_mobilenet_v3_large_fpn": {"box_score_thresh": 0.02076,},
...
@pytest.mark.parametrize(
    "model_name", ["densenet121", "densenet169", "densenet201", "densenet161"]
)
...
def test_resnet_dilation(dilate_layer_2, dilate_layer_3, dilate_layer_4):
    ...
    model = models.__dict__["resnet50"](
        replace_stride_with_dilation=(dilate_layer_2, dilate_layer_3, dilate_layer_4)
    )
...
@pytest.mark.parametrize(
    "model_name", ["mobilenet_v2", "mobilenet_v3_large", "mobilenet_v3_small"]
)
...
def test_generalizedrcnn_transform_repr():
    ...
    _indent = "\n    "
    expected_string += "{0}Normalize(mean={1}, std={2})".format(
        _indent, image_mean, image_std
    )
    expected_string += "{0}Resize(min_size=({1},), max_size={2}, ".format(
        _indent, min_size, max_size
    )
...
def test_classification_model(model_name, dev):
    ...
    _check_jit_scriptable(
        model, (x,), unwrapper=script_model_unwrapper.get(model_name, None)
    )
...
def test_segmentation_model(model_name, dev):
    ...
    def check_out(out):
        ...
        try:
            ...
            expected = torch.load(expected_file)
            torch.testing.assert_close(
                out.argmax(dim=1), expected.argmax(dim=1), rtol=prec, atol=prec
            )
    ...
    if not full_validation:
        msg = (
            ...
            "significant changes to the codebase.".format(
                test_segmentation_model.__name__
            )
        )
        warnings.warn(msg, RuntimeWarning)
        pytest.skip(msg)
...
def test_detection_model_validation(model_name):
    set_rng_seed(0)
    model = models.detection.__dict__[model_name](
        num_classes=50, pretrained_backbone=False
    )
    input_shape = (3, 300, 300)
    x = [torch.rand(input_shape)]
