...
def test_encode_png_errors():
    ...
    with pytest.raises(
        RuntimeError, match="Compression level should be between 0 and 9"
    ):
...
@pytest.mark.parametrize("shape", [(27, 27), (60, 60), (105, 105),])
@pytest.mark.parametrize("mode", [ImageReadMode.UNCHANGED, ImageReadMode.GRAY,])
...
@pytest.mark.parametrize(
    "mode", [ImageReadMode.UNCHANGED, ImageReadMode.GRAY, ImageReadMode.RGB]
)
...
@pytest.mark.skipif(
    IS_WINDOWS,
    reason=("this test fails on windows because PIL uses libjpeg-turbo on windows"),
)
@pytest.mark.parametrize(
    "img_path",
    [
