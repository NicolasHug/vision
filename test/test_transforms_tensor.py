...
def _test_transform_vs_scripted_on_batch(
    transform, s_transform, batch_tensors, msg=None
):
    torch.manual_seed(12)
...
def _test_functional_op(
    f, device, fn_kwargs=None, test_exact_match=True, **match_kwargs
):
    ...
    if test_exact_match:
        _assert_equal_tensor_to_pil(
            transformed_tensor, transformed_pil_img, **match_kwargs
        )
    else:
        _assert_approx_equal_tensor_to_pil(
            transformed_tensor, transformed_pil_img, **match_kwargs
...
def _test_class_op(
    method, device, meth_kwargs=None, test_exact_match=True, **match_kwargs
):
    ...
    if test_exact_match:
        ...
        _assert_approx_equal_tensor_to_pil(
            transformed_tensor.float(), transformed_pil_img, **match_kwargs
        )
    ...
    batch_tensors = _create_data_batch(
        height=23, width=34, channels=3, num_samples=4, device=device
    )
...
def _test_op(
    ...
    fn_kwargs=None,
    meth_kwargs=None,
    test_exact_match=True,
    **match_kwargs,
):
    _test_functional_op(
        func, device, fn_kwargs, test_exact_match=test_exact_match, **match_kwargs
    )
    _test_class_op(
...
class TestColorJitter:
    @pytest.mark.parametrize(
        "brightness", [0.1, 0.5, 1.0, 1.34, (0.3, 0.7), [0.4, 0.5]]
    )
    ...
    @pytest.mark.parametrize(
        "saturation", [0.5, 0.75, 1.0, 1.25, (0.3, 0.7), [0.3, 0.4]]
    )
...
def test_pad(m, mul, device):
    ...
    _test_functional_op(
        F.pad,
        fn_kwargs={"padding": mul * 2, "fill": fill, "padding_mode": m},
        device=device,
    )
...
class TestResize:
    ...
    def test_resize_scripted(self, dt, size, max_size, interpolation, device):
        ...
        batch_tensors = torch.randint(
            0, 256, size=(4, 3, 44, 56), dtype=torch.uint8, device=device
        )
    ...
    @pytest.mark.parametrize(
        "size", [(32,), [44,], [32,], [32, 32], (32, 32), [44, 55]]
    )
    @pytest.mark.parametrize("interpolation", [NEAREST, BILINEAR, BICUBIC])
    def test_resized_crop(self, scale, ratio, size, interpolation, device):
        tensor = torch.randint(
            0, 256, size=(3, 44, 56), dtype=torch.uint8, device=device
        )
        batch_tensors = torch.randint(
            0, 256, size=(4, 3, 44, 56), dtype=torch.uint8, device=device
        )
...
@pytest.mark.parametrize(
    "shear", [15, 10.0, (5.0, 10.0), [-15, 15], [-10.0, 10.0, -11.0, 11.0]]
)
def test_random_affine_shear(device, interpolation, shear):
    _test_random_affine_helper(
        device, degrees=0.0, interpolation=interpolation, shear=shear
    )
...
def test_random_affine_scale(device, interpolation, scale):
    _test_random_affine_helper(
        device, degrees=0.0, interpolation=interpolation, scale=scale
    )
...
def test_random_affine_translate(device, interpolation, translate):
    _test_random_affine_helper(
        device, degrees=0.0, interpolation=interpolation, translate=translate
    )
...
def test_random_affine_fill(device, interpolation, fill):
    _test_random_affine_helper(
        device, degrees=0.0, interpolation=interpolation, fill=fill
    )
...
def test_random_perspective(device, distortion_scale, interpolation, fill):
    ...
    transform = T.RandomPerspective(
        distortion_scale=distortion_scale, interpolation=interpolation, fill=fill
    )
    s_transform = torch.jit.script(transform)
...
@pytest.mark.parametrize(
    "fill", [None, 85, (10, -10, 10), 0.7, [0.0, 0.0, 0.0], [1,], 1]
)
...
def test_random_erasing_with_invalid_data():
    ...
    with pytest.raises(
        ValueError,
        match="If value is a sequence, it should have either a single value or 3",
    ):
        random_erasing(img)
...
def test_compose(device):
    ...
    transforms = T.Compose(
        [T.CenterCrop(10), T.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225)),]
    )
    ...
    transformed_tensor_script = scripted_fn(tensor)
    assert_equal(
        transformed_tensor, transformed_tensor_script, msg="{}".format(transforms)
    )

    t = T.Compose([lambda x: x,])
...
def test_random_apply(device):
    ...
    transforms = T.RandomApply([T.RandomHorizontalFlip(), T.ColorJitter(),], p=0.4)
    s_transforms = T.RandomApply(
        torch.nn.ModuleList([T.RandomHorizontalFlip(), T.ColorJitter(),]), p=0.4
    )
    ...
    if device == "cpu":
        ...
        transforms = T.RandomApply([T.ColorJitter(),], p=0.3)
        with pytest.raises(
            RuntimeError, match="Module 'RandomApply' has no attribute 'transforms'"
        ):
