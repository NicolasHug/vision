...
class TestRotate:
    ...
    @pytest.mark.parametrize(
        "fill", [None, [0, 0, 0], (1, 2, 3), [255, 255, 255], [1,], (2.0,)]
    )
    ...
    def test_rotate(self, device, height, width, center, dt, angle, expand, fill, fn):
        ...
        ratio_diff_pixels = (
            num_diff_pixels / out_tensor.shape[-1] / out_tensor.shape[-2]
        )
    ...
    def test_rotate_interpolation_type(self):
        ...
        with pytest.warns(
            UserWarning,
            match=r"Argument interpolation should be of type InterpolationMode",
        ):
...
class TestAffine:
    ...
    def test_identity_map(self, device, height, width, dt):
        ...
        assert_equal(
            tensor,
            out_tensor,
            msg="{} vs {}".format(out_tensor[0, :5, :5], tensor[0, :5, :5]),
        )
    ...
    def test_square_rotations(self, device, height, width, dt, angle, config, fn):
        ...
        if config is not None:
            assert_equal(
                torch.rot90(tensor, **config), out_tensor, check_stride=False,
            )
    ...
    def test_warnings(self, device):
        ...
        with pytest.warns(
            ...
            res1 = F.affine(
                tensor, 45, translate=[0, 0], scale=1.0, shear=[0.0, 0.0], resample=2
            )
            res2 = F.affine(
                tensor,
                45,
        ...
        with pytest.warns(
            ...
            res1 = F.affine(
                pil_img, 45, translate=[0, 0], scale=1.0, shear=[0.0, 0.0], fillcolor=10
            )
            res2 = F.affine(
                pil_img, 45, translate=[0, 0], scale=1.0, shear=[0.0, 0.0], fill=10
            )
...
def test_perspective_interpolation_warning():
    ...
    with pytest.warns(
        UserWarning, match="Argument interpolation should be of type InterpolationMode"
    ):
        res1 = F.perspective(
            tensor, startpoints=spoints, endpoints=epoints, interpolation=2
        )
        res2 = F.perspective(
            tensor, startpoints=spoints, endpoints=epoints, interpolation=BILINEAR
        )
        assert_equal(res1, res2)
...
def test_resize(device, dt, size, max_size, interpolation):
    ...
    resized_tensor = F.resize(
        tensor, size=size, interpolation=interpolation, max_size=max_size
    )
    resized_pil_img = F.resize(
        pil_img, size=size, interpolation=interpolation, max_size=max_size
    )
...
def test_resize_antialias(device, dt, size, interpolation):
    ...
    resized_tensor = F.resize(
        tensor, size=size, interpolation=interpolation, antialias=True
    )
    ...
    resize_result = script_fn(
        tensor, size=script_size, interpolation=interpolation, antialias=True
    )
    assert_equal(resized_tensor, resize_result)
...
def check_functional_vs_PIL_vs_scripted(
    fn, fn_pil, fn_t, config, device, dtype, tol=2.0 + 1e-10, agg_method="max"
):
    ...
    _assert_approx_equal_tensor_to_pil(
        rbg_tensor.float(), out_pil, tol=tol, agg_method=agg_method
    )
...
@pytest.mark.parametrize(
    "config", [{"brightness_factor": f} for f in (0.1, 0.5, 1.0, 1.34, 2.5)]
)
...
@pytest.mark.parametrize("device", cpu_and_gpu())
@pytest.mark.parametrize(
    "config", [{"threshold": threshold} for threshold in [0, 64, 128, 192, 255]]
)
...
@pytest.mark.parametrize(
    "config", [{"threshold": threshold} for threshold in [0.0, 0.25, 0.5, 0.75, 1.0]]
)
...
@pytest.mark.parametrize("dtype", (None, torch.float32, torch.float64))
@pytest.mark.parametrize(
    "config", [{"sharpness_factor": f} for f in [0.2, 0.5, 1.0, 1.5, 2.0]]
)
...
@pytest.mark.parametrize("dtype", (None, torch.float32, torch.float64))
@pytest.mark.parametrize(
    "config", [{"contrast_factor": f} for f in [0.2, 0.5, 1.0, 1.5, 2.0]]
)
...
@pytest.mark.parametrize("dtype", (None, torch.float32, torch.float64))
@pytest.mark.parametrize(
    "config", [{"saturation_factor": f} for f in [0.5, 0.75, 1.0, 1.5, 2.0]]
)
...
@pytest.mark.parametrize("dtype", (None, torch.float32, torch.float64))
@pytest.mark.parametrize(
    "config", [{"hue_factor": f} for f in [-0.45, -0.25, 0.0, 0.25, 0.45]]
)
...
def test_adjust_gamma(device, dtype, config):
    check_functional_vs_PIL_vs_scripted(
        F.adjust_gamma, F_pil.adjust_gamma, F_t.adjust_gamma, config, device, dtype,
    )
...
def test_pad(device, dt, pad, config):
    ...
    _assert_equal_tensor_to_pil(
        pad_tensor_8b, pad_pil_img, msg="{}, {}".format(pad, config)
    )
...
def test_resized_crop(device, mode):
    ...
    out_tensor = F.resized_crop(
        tensor, top=0, left=0, height=26, width=36, size=[26, 36], interpolation=mode
    )
    assert_equal(
        tensor,
        out_tensor,
    ...
    tensor, _ = _create_data(26, 36, device=device)
    out_tensor = F.resized_crop(
        tensor, top=0, left=0, height=20, width=30, size=[10, 15], interpolation=NEAREST
    )
    ...
    assert_equal(
        ...
        check_stride=False,
        msg="{} vs {}".format(
            expected_out_tensor[0, :10, :10], out_tensor[0, :10, :10]
        ),
    )
...
def test_crop(device, top, left, height, width):
    ...
    batch_tensors = _create_data_batch(16, 18, num_samples=4, device=device)
    _test_fn_on_batch(
        batch_tensors, F.crop, top=top, left=left, height=height, width=width
    )
...
def test_gaussian_blur(device, image_size, dt, ksize, sigma, fn):
    ...
    if image_size == "small":
        tensor = (
            torch.from_numpy(np.arange(3 * 10 * 12, dtype="uint8").reshape((10, 12, 3)))
            .permute(2, 0, 1)
            .to(device)
        )
    else:
    ...
    gt_key = "{}_{}_{}__{}_{}_{}".format(
        shape[-2], shape[-1], shape[-3], _ksize[0], _ksize[1], _sigma
    )
    if gt_key not in true_cv2_results:
        return

    true_out = (
        torch.tensor(true_cv2_results[gt_key])
...
def test_rgb2hsv(device):
    ...
    for _ in range(10):
        ...
        max_diff_h = (
            ((colorsys_img_h * 2 * math.pi).sin() - (ft_hsv_img_h * 2 * math.pi).sin())
            .abs()
            .max()
        )
...
def test_rgb_to_grayscale(device, num_output_channels):
    ...
    _assert_approx_equal_tensor_to_pil(
        gray_tensor.float(), gray_pil_image, tol=1.0 + 1e-10, agg_method="max"
    )
    ...
    batch_tensors = _create_data_batch(16, 18, num_samples=4, device=device)
    _test_fn_on_batch(
        batch_tensors, F.rgb_to_grayscale, num_output_channels=num_output_channels
    )
...
def test_five_crop(device):
    ...
    for transformed_batch, s_transformed_batch in zip(
        tuple_transformed_batches, s_tuple_transformed_batches
    ):
