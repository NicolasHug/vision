...
class TestConvertImageDtype:
    ...
    def test_float_to_float(self, input_dtype, output_dtype):
        ...
        torch.testing.assert_close(
            output_image_script, output_image, rtol=0.0, atol=1e-6
        )
    ...
    def test_float_to_int(self, input_dtype, output_dtype):
        ...
        if (
            input_dtype == torch.float32 and output_dtype in (torch.int32, torch.int64)
        ) or (input_dtype == torch.float64 and output_dtype == torch.int64):
    ...
    def test_dtype_int_to_int(self, input_dtype, output_dtype):
        ...
        if input_max >= output_max:
            ...
            error_term = 1 - (torch.iinfo(output_dtype).max + 1) // (
                torch.iinfo(input_dtype).max + 1
            )
...
class TestAccImage:
    ...
    def test_accimage_resize(self):
        trans = transforms.Compose(
            [transforms.Resize(256, interpolation=Image.LINEAR), transforms.ToTensor(),]
        )
        ...
        torch.testing.assert_close(
            output.numpy(), expected_output.numpy(), rtol=1e-5, atol=5e-2
        )

    def test_accimage_crop(self):
        trans = transforms.Compose([transforms.CenterCrop(256), transforms.ToTensor(),])
...
class TestToTensor:
    ...
    def test_to_tensor(self, channels):
        ...
        input_data = (
            torch.ByteTensor(channels, height, width).random_(0, 255).float().div_(255)
        )
        ...
        ndarray = np.random.randint(
            low=0, high=255, size=(height, width, channels)
        ).astype(np.uint8)
        output = trans(ndarray)
        expected_output = ndarray.transpose((2, 0, 1)) / 255.0
        torch.testing.assert_close(
            output.numpy(), expected_output, check_stride=False, check_dtype=False
        )
        ...
        output = trans(img)
        torch.testing.assert_close(
            input_data, output, check_dtype=False, check_stride=False
        )
    ...
    def test_pil_to_tensor(self, channels):
        ...
        input_data = torch.as_tensor(
            np.random.rand(channels, height, width).astype(np.float32)
        )
...
def test_randomresized_params():
    ...
    for _ in range(10):
        ...
        randresizecrop = transforms.RandomResizedCrop(
            size, scale_range, aspect_ratio_range
        )
...
def test_resize_antialias_error():
    ...
    with pytest.warns(
        UserWarning, match=r"Anti-alias option is always applied for PIL Image input"
    ):
...
class TestPad:
    def test_pad(self):
        ...
        torch.testing.assert_close(
            h_padded,
            torch.full_like(h_padded, fill_value=fill_v),
            check_stride=False,
            rtol=0.0,
            atol=eps,
        )
        torch.testing.assert_close(
            w_padded,
            torch.full_like(w_padded, fill_value=fill_v),
            check_stride=False,
            rtol=0.0,
            atol=eps,
        )
        pytest.raises(
            ValueError,
            transforms.Pad(padding, fill=(1, 2)),
    ...
    def test_pad_with_non_constant_padding_modes(self):
        ...
        assert_equal(
            edge_middle_slice,
            np.asarray([200, 200, 200, 200, 1, 0], dtype=np.uint8),
            check_stride=False,
        )
        ...
        reflect_middle_slice = np.asarray(reflect_padded_img).transpose(2, 0, 1)[0][17][
            :6
        ]
        assert_equal(
            reflect_middle_slice,
            np.asarray([0, 0, 1, 200, 1, 0], dtype=np.uint8),
            check_stride=False,
        )
        ...
        symmetric_middle_slice = np.asarray(symmetric_padded_img).transpose(2, 0, 1)[0][
            17
        ][:6]
        assert_equal(
            symmetric_middle_slice,
            np.asarray([0, 1, 200, 200, 1, 0], dtype=np.uint8),
            check_stride=False,
        )
        ...
        symmetric_neg_middle_left = np.asarray(symmetric_padded_img_neg).transpose(
            2, 0, 1
        )[0][17][:3]
        symmetric_neg_middle_right = np.asarray(symmetric_padded_img_neg).transpose(
            2, 0, 1
        )[0][17][-4:]
        assert_equal(
        ...
        assert_equal(
            symmetric_neg_middle_right,
            np.asarray([200, 200, 0, 0], dtype=np.uint8),
            check_stride=False,
        )
    ...
    def test_pad_with_mode_F_images(self):
        ...
        assert_equal(
            padded_img.size,
            [edge_size + 2 * pad for edge_size in img.size],
            check_stride=False,
        )
...
class TestToPil:
    ...
    @pytest.mark.parametrize(
        ...
        _get_1_channel_tensor_various_types(),
    )
    def test_1_channel_tensor_to_pil_image(
        self, with_mode, img_data, expected_output, expected_mode
    ):
        transform = (
            transforms.ToPILImage(mode=expected_mode)
            if with_mode
        ...
        torch.testing.assert_close(
            expected_output, to_tensor(img).numpy(), check_stride=False
        )
    ...
    def test_1_channel_float_tensor_to_pil_image(self):
        ...
        torch.testing.assert_close(
            np.array(Image.fromarray(img_data.squeeze(0).numpy(), mode="F")),
            np.array(img_F_mode),
        )
    ...
    def test_1_channel_ndarray_to_pil_image(self, with_mode, img_data, expected_mode):
        ...
        torch.testing.assert_close(
            img_data[:, :, 0], np.asarray(img).astype(img_data.dtype)
        )
    ...
    def test_2_channel_ndarray_to_pil_image(self, expected_mode):
        ...
        for i in range(2):
            torch.testing.assert_close(
                img_data[:, :, i], np.asarray(split[i]), check_stride=False
            )
    ...
    def test_2_channel_ndarray_to_pil_image_error(self):
        ...
        with pytest.raises(
            ValueError, match=r"Only modes \['LA'\] are supported for 2D inputs"
        ):
    ...
    def test_2_channel_tensor_to_pil_image(self, expected_mode):
        ...
        for i in range(2):
            torch.testing.assert_close(
                expected_output[i].numpy(), F.to_tensor(split[i]).squeeze(0).numpy()
            )
    ...
    @pytest.mark.parametrize("with_mode", [False, True])
    @pytest.mark.parametrize(
        "img_data, expected_output, expected_mode", _get_2d_tensor_various_types()
    )
    ...
    def test_3_channel_tensor_to_pil_image_error(self):
        ...
        with pytest.raises(
            ValueError, match=r"pic should be 2/3 dimensional. Got \d+ dimensions."
        ):
    ...
    def test_4_channel_tensor_to_pil_image_error(self):
        ...
        error_message_4d = (
            r"Only modes \['RGBA', 'CMYK', 'RGBX'\] are supported for 4D inputs"
        )
    ...
    def test_ndarray_bad_types_to_pil_image(self):
        ...
        with pytest.raises(
            ValueError, match=r"pic should not have > 4 channels. Got \d+ channels."
        ):
...
def test_gaussian_blur_asserts():
    ...
    with pytest.raises(
        ValueError, match=r"If kernel_size is a sequence its length should be 2"
    ):
    ...
    with pytest.raises(
        ValueError, match=r"Kernel size should be a tuple/list of two integers"
    ):
        transforms.GaussianBlur([3, 3, 3])

    with pytest.raises(
        ValueError, match=r"kernel_size should have odd and positive integers"
    ):
        F.gaussian_blur(img, [4, 4])
    with pytest.raises(
        ValueError, match=r"Kernel size value should be an odd and positive number"
    ):
    ...
    with pytest.raises(
        ValueError, match=r"If sigma is a sequence, its length should be 2"
    ):
        F.gaussian_blur(img, 3, [1, 1, 1])
    with pytest.raises(
        ValueError,
        match=r"sigma should be a single number or a list/tuple with length 2",
    ):
    ...
    with pytest.raises(
        ValueError, match=r"If sigma is a single number, it must be positive"
    ):
        transforms.GaussianBlur(3, -1.0)

    with pytest.raises(
        TypeError, match=r"kernel_size should be int or a sequence of integers"
    ):
    ...
    with pytest.raises(
        TypeError, match=r"sigma should be either float or sequence of floats"
    ):
...
def test_random_grayscale():
    ...
    p_value = stats.binom_test(
        num_gray, num_samples, p=1.0
    )  # Note: grayscale is always unchanged
    random.setstate(random_state)
    assert p_value > 0.0001
...
def test_random_order():
    ...
    random_order_transform = transforms.RandomOrder(
        [transforms.Resize(20), transforms.CenterCrop(10)]
    )
    img = transforms.ToPILImage()(torch.rand(3, 25, 25))
    num_samples = 250
    num_normal_order = 0
...
def test_random_crop():
    ...
    with pytest.raises(
        ValueError, match=r"Required crop size .+ is larger then input image size .+"
    ):
        t(img)
...
def test_center_crop_2(odd_image_size, delta, delta_width, delta_height):
    ...
    center_size = (
        min(crop_size[0], input_image_size[0]),
        min(crop_size[1], input_image_size[1]),
    )
...
def test_random_erasing():
    ...
    count_bigger_then_ones = len(
        [1 for aspect_ratio in aspect_ratios if aspect_ratio > 1]
    )
...
def test_random_rotation():
    ...
    with pytest.warns(
        UserWarning, match=r"Argument resample is deprecated and will be removed"
    ):
    ...
    with pytest.warns(
        UserWarning, match=r"Argument interpolation should be of type InterpolationMode"
    ):
...
def test_randomperspective():
    for _ in range(10):
        ...
        assert torch.nn.functional.mse_loss(
            tr_img, F.to_tensor(img)
        ) + 0.3 > torch.nn.functional.mse_loss(tr_img2, F.to_tensor(img))
...
def test_randomperspective_fill(mode):
    ...
    with pytest.raises(ValueError):
        F.perspective(
            img_conv, startpoints, endpoints, fill=tuple([fill] * wrong_num_bands)
        )
...
class TestAffine:
    ...
    def _test_transformation(
        self, angle, translate, scale, shear, pil_image, input_img
    ):
        ...
        result = F.affine(
            pil_image, angle=angle, translate=translate, scale=scale, shear=shear
        )
...
def test_random_affine():
    ...
    with pytest.raises(ValueError):
        transforms.RandomAffine(
            [-90, 90], translate=[0.2, 0.2], scale=[0.5, 0.5], shear=-7
        )
    with pytest.raises(ValueError):
        transforms.RandomAffine(
            [-90, 90], translate=[0.2, 0.2], scale=[0.5, 0.5], shear=[-10]
        )
    with pytest.raises(ValueError):
        transforms.RandomAffine(
            [-90, 90], translate=[0.2, 0.2], scale=[0.5, 0.5], shear=[-10, 0, 10]
        )
    with pytest.raises(ValueError):
        transforms.RandomAffine(
            [-90, 90], translate=[0.2, 0.2], scale=[0.5, 0.5], shear=[-10, 0, 10, 0, 10]
        )
    ...
    t = transforms.RandomAffine(
        10, translate=[0.5, 0.3], scale=[0.7, 1.3], shear=[-10, 10, 20, 40]
    )
    for _ in range(100):
        ...
        assert -10 < angle < 10
        assert (
            -img.size[0] * 0.5 <= translations[0] <= img.size[0] * 0.5
        ), "{} vs {}".format(translations[0], img.size[0] * 0.5)
        assert (
            -img.size[1] * 0.5 <= translations[1] <= img.size[1] * 0.5
    ...
    with pytest.warns(
        UserWarning, match=r"Argument fillcolor is deprecated and will be removed"
    ):
