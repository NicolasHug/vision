...
class RoIOpTester(ABC):
    ...
    def test_backward(self, device, contiguous):
        ...
        if not contiguous:
            x = x.permute(0, 1, 3, 2)
        rois = torch.tensor(
            [[0, 0, 0, 4, 4], [0, 0, 2, 3, 4], [0, 2, 2, 4, 4]],  # format is (xyxy)
            dtype=self.dtype,
            device=device,
        )
...
class TestRoIAlign(RoIOpTester):
    ...
    def test_qroialign(self, aligned, scale, zero_point, qdtype):
        ...
        qrois = torch.quantize_per_tensor(
            rois, scale=scale, zero_point=zero_point, dtype=qdtype
        )
        ...
        quantized_float_y = torch.quantize_per_tensor(
            y, scale=scale, zero_point=zero_point, dtype=qdtype
        )
        ...
        try:
            ...
            abs_diff = torch.abs(
                qy[diff_idx].dequantize() - quantized_float_y[diff_idx].dequantize()
            )
    ...
    def test_qroi_align_multiple_images(self):
        ...
        qrois = torch.quantize_per_tensor(
            rois, scale=1, zero_point=0, dtype=torch.qint8
        )
...
class TestNMS:
    ...
    def test_qnms(self, iou, scale, zero_point):
        ...
        qboxes = torch.quantize_per_tensor(
            boxes, scale=scale, zero_point=zero_point, dtype=torch.quint8
        )
        qscores = torch.quantize_per_tensor(
            scores, scale=scale, zero_point=zero_point, dtype=torch.quint8
        )
    ...
    def test_nms_cuda(self, iou, dtype=torch.float64):
        ...
        if not is_eq:
            ...
            # are duplicate
            is_eq = torch.allclose(
                scores[r_cpu], scores[r_cuda.cpu()], rtol=tol, atol=tol
            )
    ...
    def test_batched_nms_implementations(self):
        ...
        keep_vanilla = ops.boxes._batched_nms_vanilla(
        ...
        torch.testing.assert_close(
            keep_vanilla,
            keep_trick,
            msg="The vanilla and the trick implementation yield different nms outputs.",
        )
...
class TestDeformConv:
    ...
    def get_fn_args(self, device, contiguous, batch_sz, dtype):
        ...
        bias = torch.randn(
            n_out_channels, device=device, dtype=dtype, requires_grad=True
        )
    ...
    def test_backward(self, device, contiguous, batch_sz):
        ...
        gradcheck(
            func, (x, offset, mask, weight, bias), nondet_tol=1e-5, fast_mode=True
        )
        ...
        gradcheck(
            func_no_mask, (x, offset, weight, bias), nondet_tol=1e-5, fast_mode=True
        )
        ...
        gradcheck(
            lambda z, off, msk, wei, bi: script_func(
                z, off, msk, wei, bi, stride, padding, dilation
            ),
        ...
        gradcheck(
            lambda z, off, wei, bi: script_func_no_mask(
                z, off, wei, bi, stride, padding, dilation
            ),
    ...
    def test_autocast(self, batch_sz, dtype):
        with torch.cuda.amp.autocast():
            self.test_forward(
                torch.device("cuda"), contiguous=False, batch_sz=batch_sz, dtype=dtype
            )
...
class TestBoxConversion:
    def _get_box_sequences():
        ...
        box_tensor = torch.tensor(
            [[0, 0, 0, 100, 100], [1, 0, 0, 100, 100]], dtype=torch.float
        )
        box_list = [
...
class TestBox:
    def test_bbox_same(self):
        ...
        assert_equal(
            ops.box_convert(box_tensor, in_fmt="xyxy", out_fmt="xyxy"), exp_xyxy
        )
        assert_equal(
            ops.box_convert(box_tensor, in_fmt="xywh", out_fmt="xywh"), exp_xyxy
        )
        assert_equal(
            ops.box_convert(box_tensor, in_fmt="cxcywh", out_fmt="cxcywh"), exp_xyxy
        )
    ...
    def test_bbox_xyxy_xywh(self):
        ...
        exp_xywh = torch.tensor(
            [[0, 0, 100, 100], [0, 0, 0, 0], [10, 15, 20, 20], [23, 35, 70, 60]],
            dtype=torch.float,
        )
    ...
    def test_bbox_convert_jit(self):
        ...
        torch.testing.assert_close(
            scripted_cxcywh, box_cxcywh, rtol=0.0, atol=TOLERANCE
        )
...
class TestBoxArea:
    def test_box_area(self):
        def area_check(box, expected, tolerance=1e-4):
            ...
            torch.testing.assert_close(
                out, expected, rtol=0.0, check_dtype=False, atol=tolerance
            )
        ...
        for dtype in [torch.float32, torch.float64]:
            ...
            expected = torch.tensor(
                [604723.0806, 600965.4666, 592761.0085], dtype=torch.float64
            )
...
class TestBoxIou:
    def test_iou(self):
        ...
        for dtype in [torch.float16, torch.float32, torch.float64]:
            ...
            expected = torch.tensor(
                [[1.0, 0.9933, 0.9673], [0.9933, 1.0, 0.9737], [0.9673, 0.9737, 1.0]]
            )
            iou_check(
                box_tensor,
                expected,
                tolerance=0.002 if dtype == torch.float16 else 1e-4,
            )
...
class TestGenBoxIou:
    def test_gen_iou(self):
        ...
        for dtype in [torch.int16, torch.int32, torch.int64]:
            ...
            expected = torch.tensor(
                [[1.0, 0.25, -0.7778], [0.25, 1.0, -0.8611], [-0.7778, -0.8611, 1.0]]
            )
        ...
        for dtype in [torch.float16, torch.float32, torch.float64]:
            ...
            gen_iou_check(
                box_tensor,
                expected,
                tolerance=0.002 if dtype == torch.float16 else 1e-3,
            )
