...
@pytest.mark.skipif(
    sys.platform in ("win32", "cygwin"), reason="temporarily disabled on Windows"
)
def test_save_image():
...
def test_save_image_file_object():
    with tempfile.NamedTemporaryFile(suffix=".png") as f:
        ...
        assert_equal(
            F.to_tensor(img_orig),
            F.to_tensor(img_bytes),
            msg="Image not stored in file object",
        )
...
def test_draw_boxes():
    ...
    result = utils.draw_bounding_boxes(
        img, boxes, labels=labels, colors=colors, fill=True
    )
...
@pytest.mark.parametrize(
    "colors",
    [None, ["red", "blue", "#FF00FF", (1, 34, 122)], "red", "#FF00FF", (1, 34, 122)],
)
...
@pytest.mark.parametrize("colors", [None, ["red", "blue"], ["#FF00FF", (1, 34, 122)],])
...
def test_draw_segmentation_masks(colors, alpha):
    ...
    for mask, color in zip(masks, colors):
        ...
        interpolated_color = (img[:, mask] * (1 - alpha) + color[:, None] * alpha).to(
            dtype
        )
...
def test_draw_segmentation_masks_errors():
    ...
    with pytest.raises(
        ValueError, match="colors must be a tuple or a string, or a list thereof"
    ):
        bad_colors = np.array(["red", "blue"])  # should be a list
        utils.draw_segmentation_masks(image=img, masks=masks, colors=bad_colors)
    with pytest.raises(
        ValueError, match="It seems that you passed a tuple of colors instead of"
    ):
