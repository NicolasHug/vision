...
def _make_resnet_docstring(weights):
    ...
    num_layers = re.search("ResNet(.+)_Weights", name).groups()[0]
    allowed_values = ", ".join(f"``{v}``" for v in weights)

    return f"""ResNet-{num_layers} from `Deep Residual Learning for Image Recognition <https://arxiv.org/pdf/1512.03385.pdf>`__.
    ...
    Args:
        weights (:class:`~torchvision.prototype.models.{name}`, optional): The
            ...
            details. By default, no pre-trained weights are used.
