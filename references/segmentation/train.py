...
def get_transform(train):
    ...
    return (
        presets.SegmentationPresetTrain(base_size, crop_size)
        if train
        else presets.SegmentationPresetEval(base_size)
    )
...
def main(args):
    ...
    dataset, num_classes = get_dataset(
        args.data_path, args.dataset, "train", get_transform(train=True)
    )
    dataset_test, _ = get_dataset(
        args.data_path, args.dataset, "val", get_transform(train=False)
    )
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument(
        "--data-path", default="/datasets01/COCO/022719/", help="dataset path"
    )
