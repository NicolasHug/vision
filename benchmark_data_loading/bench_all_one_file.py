...
from bench_data_reading import pickle_bytesio_dp, torch_bytesio_dp, wds, tar_dp, mapstyle_ds, no_archive_dp, read_stream_wrapper, drop_file_name
...
from common import ARCHIVE_ROOT, args, bench, decode, iterate_one_epoch, JPEG_FILES_ROOT, suppress, stream_wrapper_to_tensor
...
tar_dp = make_dp(root=ARCHIVE_ROOT, archive="tar").map(drop_file_name)
...
if __name__ == "__main__":
    ...
    with suppress():
        print("tar archives DP DLV2")
        bench(iterate_one_epoch, lambda: with_DL(tar_dp.map(read_stream_wrapper), dl="v2"))
    ...
    with suppress():
        print("tar archives DP->Image.open()-> PIL Transforms DLV2")
        bench(iterate_one_epoch, lambda: with_DL(tar_dp.map(pil_loader).map(ClassificationPresetTrain(on="pil")), dl="v2"))

    with suppress():
        print("tar archives DP->ToTensor()->decode_jpeg()->Transforms() DLV2")
        bench(iterate_one_epoch, lambda: with_DL(tar_dp.map(stream_wrapper_to_tensor).map(decode).map(ClassificationPresetTrain(on="tensor")), dl="v2"))
