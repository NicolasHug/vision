    ...
    if not library_found:
        ...
        if not library_found:
            if sys.platform == "linux":
                library_found = os.path.exists(
                    "/usr/include/{0}".format(library_header)
                )
    ...
    if png_found:
        if libpng is not None:
            ...
            if png_version >= parse_version("1.6.0"):
                ...
                print(
                    "libpng installed version is less than 1.6.0, "
    ...
    if has_ffmpeg:
        try:
            ...
            if StrictVersion(ffmpeg_version) >= StrictVersion("4.3"):
                print(
                    f"ffmpeg {ffmpeg_version} not supported yet, please use ffmpeg 4.2."
                )
                has_ffmpeg = False
